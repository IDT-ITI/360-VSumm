...
def run(**kwargs):
    ...
    frames_path = kwargs['video_frames_path']
    ...
    video_names = os.listdir(frames_path)
    for video in video_names:
        video_name = os.path.basename(frames_path)
        video_name = os.path.splitext(video_name)[0]

        list_of_saliency_maps = os.listdir(os.path.join(saliency_maps_path,video))
        length = len(list_of_saliency_maps)
        frames_path = os.path.join(frames_path,video)
        salient_regions = []
  
        for i in range(length-20):  # replace  with the total number of frames

            saliency_map = cv2.imread(os.path.join(saliency_maps_path,video) + "/" + f"{i+20:04d}.png", cv2.IMREAD_GRAYSCALE)
        ...
        saliency_maps_path = os.path.join(saliency_maps_path,video)
