...
from top_k_implementation import top_k


def generate_summary(all_shot_bound, all_scores, all_nframes, all_positions, frag_selection):
    ...
    :param string frag_selection: The utilized fragment selection method for building the video summary (top-k or knapsack)
    ...
    for video_index in range(len(all_scores)):
        ...
        selected = []
        if frag_selection == "top_k":
            selected = top_k(shot_imp_scores, int(n_frames))
        elif frag_selection == "knapsack":
            selected = knapSack(final_max_length, shot_lengths, shot_imp_scores, len(shot_lengths))
        else:
            print(f"Invalid fragment selection method. Possible options: top_k and knapsack.")
