        ...
        for epoch_i in trange(self.config.n_epochs, desc='Epoch', ncols=80):
            ...
            for _ in trange(num_batches, desc='Batch', ncols=80, leave=False):
                ...
                for _ in trange(self.config.batch_size, desc='Video', ncols=80, leave=False):
                    frame_features, target, saliency_scores = next(iterator)

                    frame_features = frame_features.to(self.config.device)
                    target = target.to(self.config.device)

                    frame_saliency = saliency_scores.squeeze(0).to(self.config.device)

                    weighted_frame_features = frame_saliency.unsqueeze(1) * frame_features.squeeze(0)
