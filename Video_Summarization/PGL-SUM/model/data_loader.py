...
class VideoData(Dataset):
    def __init__(self, mode, video_type, split_index):
        ...
        """
        self.mode = mode
        self.name = video_type
        self.filename = '.../data/360VSumm.h5'
        self.splits_filename = ['.../data/' + self.name + '_splits.json']
        ...
        for video_name in self.split[self.mode + '_keys']:
            ...
            gtscore_list = gtscore.tolist()
            picks = torch.Tensor(np.array(hdf[video_name + '/picks']))
            picks_list = picks.tolist()
            gtscore_sampled = [gtscore_list[int(i)] for i in picks_list]
            gtscores = torch.tensor(gtscore_sampled)
            sal_scores = torch.Tensor(np.array(hdf[video_name + '/saliency_scores']))

            self.list_frame_features.append(frame_features)
            self.list_gtscores.append(gtscores)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'test':
            ...
            return frame_features, gtscore, saliency_scores
