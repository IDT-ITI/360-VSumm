...
class Solver(object):
    def __init__(self, config=None, train_loader=None, test_loader=None):
        """ Class that Builds, Trains and Evaluates the CA-SUM model. """
        ...
        for epoch_i in trange(self.config.n_epochs, desc='Epoch', ncols=80):
            ...
            for _ in trange(num_batches, desc='Batch', ncols=80, leave=False):
                ...
                for _ in trange(self.config.batch_size, desc='Video', ncols=80, leave=False):
                    frame_features, _, _ = next(iterator)
    ...
    def evaluate(self, epoch_i, save_weights=False):
        ...
        out_scores_dict = {}
        for frame_features, video_name, _ in tqdm(self.test_loader, desc='Evaluate', ncols=80, leave=False):
            # [seq_len, input_size]
