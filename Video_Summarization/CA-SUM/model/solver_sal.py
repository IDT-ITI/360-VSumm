...
class Solver(object):
    def __init__(self, config=None, train_loader=None, test_loader=None):
        """ Class that Builds, Trains and Evaluates the CA-SUM-sal model. """
    ...
    def build(self):
        """ Function for constructing the CA-SUM-sal model, its key modules and parameters. """
        # Model creation
    ...
    def length_regularization_loss(self, scores):
        ...
        :param torch.Tensor scores: Frame-level importance scores, produced by our CA-SUM-sal model.
    ...
    def train(self):
        """ Main function to train the CA-SUM-sal model. """
    ...
    def evaluate(self, epoch_i, save_weights=False):
        ...
        out_scores_dict = {}
        for frame_features, video_name, saliency_scores in tqdm(self.test_loader, desc='Evaluate', ncols=80, leave=False):
            # [seq_len, input_size]
            frame_features = frame_features.view(-1, self.config.input_size).to(self.config.device)
            frame_saliency = saliency_scores.to(self.config.device)  # [T]

            with torch.no_grad():
                weighted_frame_features = frame_saliency.unsqueeze(1) * frame_features
                scores, _ = self.model(weighted_frame_features)  # [1, seq_len]
